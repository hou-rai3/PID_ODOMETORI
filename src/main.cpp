...
int pwm_power[4] = {};         // pwm計算の配列
int count_hozon[4] = {};       // ロリコンの今回のカウント合計値
...
void Odometry(float &x_mtere, float &y_mtere, float &theta)
{
    ...
    theta += 2 * M_PI / -34552 * dtheta_rad;                              // 360度を一回転分のカウントで割り、ラジアンにする//1回転あたり34552回転
...
{
    const float r_adjust = 8000;
    const float theta_adjust = 2000;
    const float pwm_angle = 45;
    float r = hypot(Omni_x_PID, Omni_y_PID);
    float theta_2 = atan2(Omni_y_PID, Omni_x_PID);
    //                                          |< + piteh(姿勢角)
    for (int i = 0; i < 4; ++i)
    {
        pwm_power[i] = r * r_adjust * cos(theta_2 + ((pwm_angle - 90 * i) * M_PI / 180)) + Omni_theta_PID * theta_adjust;
    }

    // 値をint16_tにキャストして範囲をクリップする
    for (int i = 0; i < 4; ++i)
    {
        pwm[i] = std::clamp(pwm_power[i], -8000, 8000);
    }
}
